...
def map_to_icd(user_input: str, system: str = "siddha"):
    ...
    elif system == "ayurveda-sat":
        tm_out = search_ayurveda_sat(user_input)
    ...
    tm_definition = tm_candidates[0].get("definition", "")

    # STEP 2 — Build search blob for ICD
    # Include definition if available for better ICD mapping
    if tm_definition and tm_definition != "No description available.":
        search_blob = f"{user_input} {tm_term} {tm_definition} {tm_reason}"
    else:
    ...
    # Normalize system name for key (replace hyphen with underscore)
    system_key = system.replace("-", "_")
    
    return {
        "input": user_input,
        "system": system,
        f"{system_key}_candidates": tm_candidates,
